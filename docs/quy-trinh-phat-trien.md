...
## 📅 SƠ ĐỒ TIẾN TRÌNH 8 GIAI ĐOẠN (PROJECT LIFECYCLE)
...
* Hệ thống sẽ tự động lấy trung bình cộng tiến trình của 8 bước để tính toán ra **Tiến độ tổng thể** của dự án thực tế theo công thức:
  $$\text{Tiến độ tổng thể} = \text{Round}\left(\frac{\sum_{i=1}^{8} \text{Tiến độ bước } i}{8}\right)$$
